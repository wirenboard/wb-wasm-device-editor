...
2. Sets `fw_version` on the device if provided (needed for firmware-gated register definitions).
...
<TabContent activeTab={activeSettingsTab} tabId={RUNTIME_VIEW_TAB_ID}>
  <RuntimeView
    ...
    fwVersion={getDevice().fw?.version}
  />
</TabContent>
```

Props no longer include `configParams` or `evaluateCondition` — condition filtering is entirely server-side (C++).
...
| `fw_version` passed in request | Some registers are gated by firmware version in templates. Without this, the WASM module would not generate the correct register configs. |
...
| `wasm/src/wasm_module.cpp` | `DeviceLoad()` entry point, Emscripten binding, `fw_version` passthrough |
...
| `evaluateCondition` / `configParams` props on `RuntimeView` | Carried unsaved editor state for JS filtering, replaced by actual device values read in C++ |
