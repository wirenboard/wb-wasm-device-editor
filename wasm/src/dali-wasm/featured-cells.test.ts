...
import {
  FEATURED_LIMIT,
  pickFeatured,
} from '@/pages/settings/configs/dali/components/device-controls/device-controls';
import type Cell from '@/stores/devices/cell';

/** The slice of Cell the heuristic reads. */
const cell = (
  controlId: string,
  name: string,
  type: string,
  ...
  ({ id: `dev/${controlId}`, controlId, name, type, readOnly, order } as unknown as Cell);
...
describe('mergeReadbacks', () => {
  ...
  it('folds a featured reading into its setpoint control', async () => {
    const { mergeReadbacks, pickFeatured } = await import(
      '@/pages/settings/configs/dali/components/device-controls/device-controls'
    );
    ...
    expect(shown).not.toContain('current_rgb');
    const wanted = merged.find((entry) => entry.cell.controlId === 'wanted_level');
    expect(wanted?.readback?.controlId).toBe('actual_level');
    const rgb = merged.find((entry) => entry.cell.controlId === 'set_rgb');
    expect(rgb?.readback?.controlId).toBe('current_rgb');
  });

  it('leaves an unpaired reading alone', async () => {
    const { mergeReadbacks, pickFeatured } = await import(
      '@/pages/settings/configs/dali/components/device-controls/device-controls'
    );
    const merged = mergeReadbacks(pickFeatured(all), all);
